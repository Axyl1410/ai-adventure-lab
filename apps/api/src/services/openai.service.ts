...
import { promptCoachResultSchema, type PromptCoachResult } from "@ai-adventure/shared";
...
export class OpenAIService {
  async chat(sessionId: string | undefined, message: string, ageGroup: string) {
    ...
    if (!safety.safe) return safety.message ?? "Mình chuyển sang chủ đề học tập an toàn nhé.";
    ...
    try {
      ...
      const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
        { role: "system", content: `${buddyBotSystemPrompt}\nNhóm tuổi: ${ageGroup}.` }
      ];
      ...
      return response.choices[0]?.message.content?.trim() || this.localBuddyReply(message);
    } catch (error) {
  ...
  async promptFeedback(prompt: string, ageGroup: string): Promise<PromptCoachResult> {
    ...
    try {
      const response = await client.chat.completions.create({
        model: textModel(),
        messages: [
          { role: "system", content: `${promptCoachSystemPrompt}\nNhóm tuổi: ${ageGroup}.` },
      ...
      const outputSafety = safetyService.checkText(`${parsed.data.feedback}\n${parsed.data.improvedPrompt}`);
  ...
  private localPromptFeedback(prompt: string): PromptCoachResult {
    ...
    return {
      ...
      feedback: "Prompt của em đã có ý chính. Em có thể thêm người nghe và định dạng mong muốn để AI trả lời dễ hiểu hơn.",
