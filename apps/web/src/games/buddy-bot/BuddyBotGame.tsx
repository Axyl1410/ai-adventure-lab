...
function formatMessageText(text: string) {
  ...
  return lines.map((line, lineIdx) => {
    ...
    const parsedLine = parts.map((part, partIdx) => {
      if (partIdx % 2 === 1) {
        return <strong key={partIdx} className="font-extrabold text-purpleLab">{part}</strong>;
      }
      return part;
    });
    ...
    if (isListItem) {
      return (
        <div key={lineIdx} className="flex items-start gap-1.5 my-1 pl-2 text-ink">
...
export function BuddyBotGame() {
  ...
  const [messages, setMessages] = useState([{ role: "assistant", text: "Chào em! 👋 Mình là Buddy Bot. Em muốn khám phá điều gì về AI nào?" }]);
  ...
  async function send(text = input) {
    ...
    } else if (lowerText.includes("toán") || lowerText.includes("đố") || lowerText.includes("câu đố")) {
      nextChips = [
    ...
    } else if (lowerText.includes("ai") || lowerText.includes("trí tuệ nhân tạo") || lowerText.includes("máy học")) {
      nextChips = [
    ...
    } else if (lowerText.includes("prompt") || lowerText.includes("câu lệnh") || lowerText.includes("tranh") || lowerText.includes("vẽ")) {
      nextChips = [
  ...
  return (
    <GameShell title="💬 Buddy Bot Trò Chuyện" subtitle="Trò chuyện với robot học tập an toàn." instruction="Trò chuyện với robot học tập Buddy Bot.">
      <section className="grid flex-1 gap-5 lg:grid-cols-[220px_1fr]">
        ...
        <div className="lab-card flex min-h-[62dvh] flex-col bg-white/80 p-3 sm:p-5 lg:min-h-[560px]">
          ...
          {/* Chat messages */}
          <div ref={chatRef} className="min-h-[300px] flex-1 overflow-y-auto rounded-3xl border border-white/40 bg-cream/60 p-3 scroll-smooth sm:p-4">
            {messages.map((msg, i) => (
              <div key={i} className={`mb-3 flex max-w-[92%] gap-2.5 sm:max-w-[85%] ${msg.role === "user" ? "ml-auto flex-row-reverse" : ""}`}>
                {msg.role === "assistant" && (
                ...
                <div className="flex flex-col gap-1.5 min-w-0">
                  <div className={`rounded-3xl p-4 font-semibold leading-relaxed ${
                    msg.role === "user"
                  ...
                  {msg.role === "assistant" && (
                    <div className="self-start pl-1">
                      <TTSButton text={msg.text} compact autoPlay={i === messages.length - 1} />
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="sticky bottom-0 mt-2 flex gap-2.5 rounded-2xl bg-white/85 pt-2 pb-[env(safe-area-inset-bottom)] backdrop-blur sm:mt-3">
            <label className="sr-only" htmlFor="buddy-chat-input">Câu hỏi của em</label>
            <input
              id="buddy-chat-input"
