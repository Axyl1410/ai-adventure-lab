...
const STICKERS = [
  { id: "detective", name: "Thám tử nhí", image: stickerDetective, hint: "Đạt >= 4 điểm trong game Thám tử AI" },
  { id: "robot", name: "Kỹ sư máy học", image: stickerRobot, hint: "Đạt >= 5 điểm trong Dạy Robot Học hoặc hoàn thành Huấn Luyện AI Mini" },
  { id: "oops", name: "Thám tử phản biện", image: stickerOops, hint: "Đạt >= 3 điểm trong game AI Có Thể Sai" },
  { id: "prompt", name: "Pháp sư Prompt", image: stickerPrompt, hint: "Đạt >= 80 điểm trong game Phép thuật câu lệnh" },
  { id: "artist", name: "Họa sĩ AI", image: stickerArtist, hint: "Tạo thành công tranh trong Xưởng Tranh AI" }
];
...
const quickStats = [
  ...
  { label: "Không cần tài khoản", value: "✓", tone: "bg-greenLab/20 text-green-700" }
];
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-5 sm:py-8">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-soft backdrop-blur-md sm:p-7 lg:p-8">
        <div className="absolute inset-x-8 top-0 h-1 rounded-full bg-gradient-to-r from-skyLab via-purpleLab to-pinkLab" aria-hidden="true" />
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-base font-bold leading-relaxed text-muted sm:text-lg">
                Cùng Buddy Bot khám phá AI qua trò chơi, hình ảnh, giọng nói và thử thách thông minh!
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3">
              {quickStats.map((stat) => (
                <div key={stat.label} className={`rounded-3xl px-4 py-3 font-black shadow-sm ${stat.tone}`}>
                  <div className="text-2xl leading-none">{stat.value}</div>
                  <div className="mt-1 text-xs uppercase tracking-wide">{stat.label}</div>
                </div>
              ))}
            </div>

            <div className="flex flex-wrap gap-3">
              <a href="#games" className="big-button inline-flex items-center gap-2 bg-gradient-to-r from-ink to-ink/90 text-white">
              ...
              >
                <Trophy className="h-5 w-5 text-orange-500" /> Sổ Sticker ({unlockedStickers.length}/5)
              </button>
            </div>
          </motion.div>
      ...
      <section id="games" className="mt-8">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-sm font-black uppercase tracking-wide text-purple-700">Chọn một cuộc phiêu lưu</p>
            <h2 className="text-3xl font-black text-ink">Trò chơi AI có minh họa</h2>
          </div>
          <p className="max-w-xl text-sm font-bold leading-relaxed text-muted">
            Mỗi trò chơi có hình ảnh riêng để học sinh dễ nhận biết mục tiêu trước khi bắt đầu.
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className={`relative h-48 overflow-hidden ${card.imageTone}`}>
                  <div className={`absolute inset-x-0 top-0 h-2 bg-gradient-to-r ${card.color}`} aria-hidden="true" />
                  <img
                    src={card.image}
                    alt={card.alt}
                ...
                <div className="space-y-4 p-5">
                  <div className="min-h-[106px]">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="text-2xl font-black leading-tight text-ink">{card.title}</h3>
                      <span className="shrink-0 rounded-2xl bg-cream px-3 py-1 text-xs font-black text-muted">
                        {card.time}
                      </span>
                    </div>
                    <p className="mt-3 text-sm font-bold leading-relaxed text-muted">{card.desc}</p>
                  </div>
                  ...
                  <Link
                    ...
                    to={card.to}
                  >
                    Bắt đầu <ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" />
                  </Link>
                </div>
              </motion.article>
            );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="shrink-0 pr-10">
                <h2 id="sticker-book-title" className="flex items-center gap-2 border-b border-cream pb-3 text-2xl font-black text-ink">
                  <Trophy className="h-7 w-7 text-orange-500" /> Sổ thu thập Sticker Buddy Bot
                </h2>
                <p className="mt-2 text-sm font-bold text-muted">
                  Chơi các trò chơi và hoàn thành thử thách để thu thập trọn bộ sticker Buddy Bot.
                </p>
              </div>
                      ...
                      <div className="relative mb-3 flex h-20 w-20 items-center justify-center">
                        <img
                          ...
                          className={`h-full w-full object-contain transition-all duration-300 ${
                            isUnlocked ? "drop-shadow-md" : "grayscale opacity-40 blur-[0.5px]"
                          }`}
                        />
                        {!isUnlocked && (
                          <span className="absolute text-lg" role="img" aria-label="locked">
                            🔒
                          </span>
                        )}
                      </div>
                      <div>
                        <h3 className={`text-base font-black ${isUnlocked ? "text-ink" : "text-muted"}`}>{sticker.name}</h3>
