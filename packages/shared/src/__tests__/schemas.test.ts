...
describe("sessionSchema", () => {
  it("nhận dữ liệu hợp lệ", () => {
    const result = sessionSchema.safeParse({ nickname: "BéNam", ageGroup: "6-8" });
...
describe("chatSchema", () => {
  it("nhận message hợp lệ", () => {
    const result = chatSchema.safeParse({ sessionId: "ck12345678901234567890123", message: "AI là gì?" });
    expect(result.success).toBe(true);
  });
  it("từ chối message quá dài", () => {
    const result = chatSchema.safeParse({ sessionId: "ck12345678901234567890123", message: "x".repeat(801) });
    expect(result.success).toBe(false);
  });
  it("từ chối message rỗng", () => {
    const result = chatSchema.safeParse({ sessionId: "ck12345678901234567890123", message: "   " });
    expect(result.success).toBe(false);
  });
});
...
describe("progressSchema", () => {
  it("nhận progress hợp lệ", () => {
    const result = progressSchema.safeParse({ sessionId: "ck12345678901234567890123", gameKey: "ai-detective", score: 4, maxScore: 5 });
    expect(result.success).toBe(true);
  });
  it("từ chối score âm", () => {
    const result = progressSchema.safeParse({ sessionId: "ck12345678901234567890123", gameKey: "ai-detective", score: -1, maxScore: 5 });
    expect(result.success).toBe(false);
  });
  it("từ chối maxScore = 0", () => {
    const result = progressSchema.safeParse({ sessionId: "ck12345678901234567890123", gameKey: "ai-detective", score: 0, maxScore: 0 });
    expect(result.success).toBe(false);
  });
});
...
describe("imageGenerateSchema", () => {
  const base = {
    ...
    details: { subject: "chú mèo", setting: "lớp học", colors: ["xanh da trời"], mood: "vui vẻ" }
  };
