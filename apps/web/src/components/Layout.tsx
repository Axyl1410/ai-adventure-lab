...
export function Layout() {
  ...
  return (
    <div className={`relative overflow-x-hidden ${isCompactLayout ? "min-h-dvh flex flex-col" : "min-h-screen flex flex-col justify-between"}`}>
      {/* Floating decorations — background only */}
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
        <div className="floating-star" style={{ top: "8%", left: "5%", animationDelay: "0s" }} />
        <div className="floating-star" style={{ top: "15%", right: "8%", animationDelay: "1.5s" }} />
        <div className="floating-star" style={{ top: "55%", left: "3%", animationDelay: "3s" }} />
        <div className="floating-star" style={{ top: "70%", right: "6%", animationDelay: "0.8s" }} />
        <div className="floating-cloud" style={{ top: "12%", left: "15%", animationDelay: "0s" }} />
        <div className="floating-cloud" style={{ top: "45%", right: "12%", animationDelay: "4s" }} />
        <div className="floating-cloud" style={{ top: "80%", left: "60%", animationDelay: "2s" }} />
        <div className="floating-bubble" style={{ bottom: "20%", left: "10%", animationDelay: "1s" }} />
        <div className="floating-bubble" style={{ bottom: "35%", right: "15%", animationDelay: "3.5s" }} />
        <div className="floating-bubble" style={{ top: "30%", left: "70%", animationDelay: "2.5s" }} />
      </div>
      ...
      <header className="sticky top-0 z-20 border-b border-white/40 bg-white/70 backdrop-blur-lg shadow-sm flex-shrink-0">
        <div className="mx-auto grid max-w-7xl gap-2 px-3 py-2 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-3 sm:px-4 sm:py-3">
          <Link to="/" className="flex min-w-0 items-center gap-2 group transition-transform hover:scale-[1.01] sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <span className="hidden sm:block text-xs font-bold text-muted -mt-0.5">🌈 Rainbow Robot Classroom</span>
            </div>
          </Link>
          <nav className="grid grid-cols-2 items-center gap-2 text-xs font-bold sm:flex sm:flex-wrap sm:gap-2.5 sm:text-sm">
            ...
            >
              {bgmEnabled ? <Music className="h-4.5 w-4.5 text-orangeLab animate-pulse" /> : <VolumeX className="h-4.5 w-4.5 text-muted" />}
              <span className="truncate">{bgmEnabled ? "Nhạc: Bật" : "Nhạc: Tắt"}</span>
            </button>
            ...
            >
              ...
              <span className="truncate">{voiceGender === "female" ? "Cô giáo" : "Thầy giáo"}</span>
            </button>
            <span className="col-span-2 min-h-10 rounded-2xl bg-greenLab/15 border border-greenLab/25 px-3 py-2 shadow-sm text-ink flex items-center justify-center gap-1.5 sm:col-span-1 sm:min-h-0 sm:px-4 sm:py-2.5">
              <ShieldCheck className="h-4 w-4 text-green-600" /> An toàn cho học sinh
            </span>
          </nav>
        </div>
      </header>

      <div className={`relative z-10 ${isCompactLayout ? "flex-1 min-h-0 flex flex-col" : ""}`}>
        <Outlet />
      </div>
