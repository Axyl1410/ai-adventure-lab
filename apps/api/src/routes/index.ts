...
routes.post(
  ...
  ah(async (req, res) => {
    ...
    const progress = await prisma.gameProgress.create({
      data: { ...req.body, metadata: JSON.stringify(req.body.metadata) },
    });
...
routes.post(
  ...
  ah(async (req, res) => {
    ...
    const generated = await imageService.generateImageFile(
      promptResult.prompt,
      record.id,
      req.body.style
    );
...
routes.post(
  "/teacher/activities",
  requireTeacher,
  validateBody(teacherActivitySchema),
  ah(async (req, res) => {
    const activity = await prisma.teacherActivity.create({
      data: { ...req.body, config: JSON.stringify(req.body.config) },
    });
...
routes.put(
  ...
  validateBody(teacherActivitySchema),
  ah(async (req, res) => {
    const existing = await prisma.teacherActivity.findUnique({
      where: { id: req.params.id },
      select: { id: true },
    });
    if (!existing) {
    ...
    const activity = await prisma.teacherActivity.update({
      where: { id: req.params.id },
      data: { ...req.body, config: JSON.stringify(req.body.config) },
    });
...
function safeCsv(value: string) {
  const sanitized = /^[=+\-@]/u.test(value) ? `'${value}` : value;
