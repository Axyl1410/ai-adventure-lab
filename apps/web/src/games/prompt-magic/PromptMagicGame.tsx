...
export function PromptMagicGame() {
  ...
  const [selected, setSelected] = useState({ role: blocks.role[0], task: blocks.task[0], audience: blocks.audience[0], style: blocks.style[0], format: blocks.format[0] });
  const [result, setResult] = useState<Awaited<ReturnType<typeof promptFeedback>> | null>(null);
  ...
  function resetLevel() {
    setLevel(null);
    setSelected({ role: blocks.role[0], task: blocks.task[0], audience: blocks.audience[0], style: blocks.style[0], format: blocks.format[0] });
    setResult(null);
  }

  if (!level) {
    return (
      <GameShell title="Prompt Magic" subtitle="Ghép prompt để hướng dẫn AI." instruction="Chọn cấp độ rèn luyện phép thuật viết câu lệnh nhé!">
        <div className="lab-card mx-auto max-w-2xl p-8 text-center space-y-6 bg-white/80">
          <div className="text-6xl mb-2">🪄</div>
          <h2 className="text-3xl font-black text-ink">Pháp sư viết câu lệnh (Prompt)</h2>
          <p className="font-bold text-muted text-lg">Prompt là câu lệnh để hướng dẫn AI làm việc cho em. Chọn cấp độ viết nhé:</p>
  ...
  const activeCategories = level === "easy" 
    ? Object.entries(blocks).filter(([key]) => ["task", "audience", "style"].includes(key))
    : Object.entries(blocks);

  return (
    <GameShell title={`Prompt Magic - Chế độ ${level === "easy" ? "Tập sự" : "Pháp sư"}`} subtitle="Ghép prompt để hướng dẫn AI." instruction="Chọn từng mảnh ghép bên dưới để tạo nên một câu lệnh hoàn hảo và gửi cho Prompt Coach chấm điểm nhé!">
          ...
          {activeCategories.map(([key, options]) => (
            <div key={key} className="space-y-3">
              ...
              <div className="flex flex-wrap gap-3">
                {options.map((option) => {
                  const isSelected = selected[key as keyof typeof selected] === option;
                  return (
                    <button
                      key={option}
                    ...
                    >
                      {isSelected && <Check className="h-5 w-5 text-ink stroke-[3px]" />}
                      {option}
                    </button>
                  );
                })}
        ...
        <aside className="lab-card flex flex-col justify-between border-white/70 bg-white/85 p-4 sm:p-6">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-tr from-purpleLab to-pinkLab text-white shadow-soft"><Wand2 className="h-6 w-6" /></span>
              <h2 className="text-2xl font-black text-ink tracking-tight">Prompt của em</h2>
            </div>
            <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center">
              <p className="relative flex-grow break-words rounded-3xl border border-yellowLab/30 bg-cream/70 p-4 text-base font-black italic leading-relaxed text-ink sm:p-5 sm:text-lg">
                "{prompt}"
                {sparkle && (
                  <span className="absolute -top-3 -right-3 text-3xl animate-sparkle pointer-events-none">✨</span>
                )}
              </p>
              <TTSButton text={prompt} compact />
            </div>
            <button className="big-button w-full bg-ink hover:bg-ink/90 text-white flex items-center justify-center gap-2 shadow-md text-lg py-3.5" onClick={() => void submit()} disabled={loading}>
              <Sparkles className="h-5 w-5 text-yellowLab fill-yellowLab" /> Gửi Prompt Coach
            </button>
          </div>
          
          {loading && <div className="mt-5"><LoadingBuddy /></div>}
          ...
          {result && (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-lg font-black text-ink">Kết quả đánh giá:</span>
                <motion.span 
                ...
                >
                  <Star className="h-4.5 w-4.5 fill-white text-yellowLab" /> {result.score}/100
                  <span className="absolute -top-1 -right-1 text-xs animate-sparkle text-yellow-200">✨</span>
                </motion.span>
              </div>
              ...
              <p className="font-semibold text-muted text-sm leading-relaxed border-l-4 border-purpleLab pl-3 py-1">{result.feedback}</p>
              
              <div className="space-y-1.5 bg-skyLab/10 border border-skyLab/20 rounded-2xl p-3">
                <p className="text-xs font-black text-sky-800 uppercase tracking-wider flex items-center gap-1"><ShieldCheck className="h-4 w-4" /> Prompt gợi ý hay hơn:</p>
                <p className="font-bold text-ink text-sm leading-relaxed">"{result.improvedPrompt}"</p>
              </div>

              <button className="big-button w-full bg-cream border border-yellowLab/50 text-ink mt-2" onClick={resetLevel}>
