...
const scenarios: Scenario[] = [
  { emoji: "🔐", text: "AI hỏi mật khẩu tài khoản học tập của em.", answer: "Không nên", explain: "Mật khẩu là bí mật. Không chia sẻ cho AI hoặc người lạ." },
  { emoji: "📚", text: "Em nhờ AI giải thích bài khoa học bằng 3 ý ngắn.", answer: "Nên làm", explain: "Đây là cách dùng AI hỗ trợ học tập an toàn." },
  { emoji: "🏥", text: "AI khuyên uống thuốc khi em bị đau bụng.", answer: "Hỏi người lớn", explain: "Thông tin sức khỏe cần hỏi bố mẹ, thầy cô hoặc bác sĩ." },
  { emoji: "🖼️", text: "Em muốn tạo tranh robot trong lớp học và ghi nhãn là hình AI.", answer: "Nên làm", explain: "Tạo tranh an toàn và ghi nhãn rõ ràng là thói quen tốt." },
  { emoji: "🏠", text: "AI yêu cầu em nhập địa chỉ nhà để kể chuyện hay hơn.", answer: "Không nên", explain: "Địa chỉ nhà là thông tin riêng tư." },
  { emoji: "📰", text: "AI nói một tin lạ trên mạng là đúng 100%.", answer: "Hỏi người lớn", explain: "Tin lạ cần kiểm tra với nguồn đáng tin cậy và người lớn." },
  { emoji: "🧠", text: "Em hỏi AI: Vì sao AI có thể sai?",
    answer: "Nên làm", explain: "Học cách hiểu giới hạn của AI giúp em dùng AI thông minh hơn." },
  { emoji: "👤", text: "Em tải ảnh mặt thật của bạn lên để AI chỉnh sửa.", answer: "Không nên", explain: "Ảnh người thật cần được đồng ý và bảo vệ riêng tư." }
];
...
const choiceStyle: Record<Choice, { emoji: string; className: string }> = {
  ...
  "Hỏi người lớn": { emoji: "🙋", className: "from-yellowLab to-orangeLab text-ink" }
};
...
export function AiSafetyQuestGame() {
  ...
  function answer(choice: Choice) {
    ...
    setScore(nextScore);
    setFeedback(`${correct ? "Quyết định an toàn!" : "Mình cân nhắc lại nhé."} ${current.explain}`);

    if (index === deck.length - 1) {
      setDone(true);
      if (nextScore >= 5) unlockSticker("oops");
      if (session) void saveProgress(session.id, "ai-safety-quest", nextScore, deck.length);
    }
  }
        ...
        <AnimatePresence mode="wait">
          {current && (
            <motion.div key={index} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              <p className="text-sm font-black text-muted mb-3">🛡️ Nhiệm vụ {index + 1} / {deck.length}</p>
              ...
              {!feedback ? (
                  ...
                  {choices.map((choice) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileTap={{ scale: 0.97 }}
                      >
                        <span className="mr-2 text-2xl">{style.emoji}</span>{choice}
                      </motion.button>
                    );
                  })}
                </div>
                ...
                <div className="space-y-5">
                  {feedback.startsWith("Quyết") ? <HappyFeedback text={feedback} /> : <TryAgainFeedback text={feedback} />}
                  {!done ? (
                    <button className="big-button bg-ink text-white text-lg" onClick={next}>Nhiệm vụ tiếp theo ➡️</button>
                  ) : (
