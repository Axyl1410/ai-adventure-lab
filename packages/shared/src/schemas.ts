...
export const cuidSchema = z.string().trim().regex(/^c[a-z0-9]{24}$/i, "ID không hợp lệ.");
...
export const ttsVoiceSchema = z.string().trim().regex(/^[a-zA-Z0-9_-]{1,32}$/, "Giọng đọc không hợp lệ.");

export const imageThemes = ["Động vật dễ thương", "Robot trong lớp học", "Hành tinh và vũ trụ", "Khu rừng cầu vồng", "Đồ vật học tập", "Nhân vật truyện cổ tích không có bản quyền", "Biển và sinh vật biển", "Thành phố tương lai thân thiện"] as const;
export const imageStyles = ["Tranh hoạt hình", "Tranh màu nước", "Sticker vui nhộn", "Poster lớp học", "Sách tranh thiếu nhi", "Pixel art đơn giản"] as const;
export const imageColors = ["xanh da trời", "vàng", "hồng", "xanh lá", "tím", "cam"] as const;
...
export const imageGenerateSchema = z.object({
  ...
  details: z.object({
    ...
    colors: z.array(z.enum(imageColors)).min(1).max(4).default(["xanh da trời"]),
